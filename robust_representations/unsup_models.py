...
def make_and_restore_model(args, dataset):
    ...
    dim_local = MI1x1ConvNet(9, 3)

    # estimator part 2: Z to H space
    dim_global = MI1x1ConvNet(3, 3)
...
class BigEncoder(nn.Module):
    ...
    def draw(self, input, private_labels, target):
        ...
        x3 = x_all[np.where(targets == 2)]
        y1 = y_all[np.where(targets == 0)]
        y2 = y_all[np.where(targets == 1)]
        y3 = y_all[np.where(targets == 2)]
        z1 = z_all[np.where(targets == 0)]
        z2 = z_all[np.where(targets == 1)]
        z3 = z_all[np.where(targets == 2)]
        labels1 = labels[np.where(targets == 0)]
        labels2 = labels[np.where(targets == 1)]
        labels3 = labels[np.where(targets == 2)]
        color1 = self.get_color(labels1)
        color2 = self.get_color(labels2)
        color3 = self.get_color(labels3)
        fig = plt.figure()
        ax = fig.add_subplot(projection='3d')
        ax.scatter(x1, y1, z1, c=color1, marker='.')
        ax.scatter(x2, y2, z2, c=color2, marker='x')
        ax.scatter(x3, y3, z3, c=color3, marker='*')
        plt.locator_params(axis='both', nbins=3)
        #plt.xlim(0.1115, 0.1135)
        #plt.ylim(0.075, 0.0775)
        #ax.set_zlim(0.24, 0.25)
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
        ax.get_zaxis().set_visible(False)
        ax.view_init(0, 150, 0)
        #plt.axis('off')
        new_x1 = y1*np.sin((9*np.pi)/6) + x1*np.cos((8.5*np.pi)/6)
        new_x2 = y2*np.sin((9*np.pi)/6) + x2*np.cos((8.5*np.pi)/6)
        new_x3 = y3*np.sin((9*np.pi)/6) + x3*np.cos((8.5*np.pi)/6)
        ax2 = fig.add_subplot()
        ax2.scatter(new_x1, z1, c=color1, marker='.', alpha=0.5)
        ax2.scatter(new_x2, z2, c=color2, marker='x', alpha=0.5)
        ax2.scatter(new_x3, z3, c=color3, marker='*', alpha=0.5)
        plt.xlim(0.30, 0.40)
        #plt.title('Encoder Plot')
        #plt.scatter(new_x1, z1)
        plt.show()
        print(1)

    def update_plot(self, input, private_labels, target):
        rep_out = self.encoder(input)
        x = rep_out[:, 0]
        y = rep_out[:, 1]
        z = rep_out[:, 2]
    ...
    def privacy_defense(self, outs, target, criterion=nn.CrossEntropyLoss()):
        ...
        group1 = outs[target == 0]
        group2 = outs[target == 1]
        distances = 0
        k = 0
