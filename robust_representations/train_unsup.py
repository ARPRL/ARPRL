...
def train_model(args, atm, datasets, *, checkpoint=None, store=None):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        #atm.attacker.model.draw(input, private_labels, target)
...
def _model_loop(args, loop_type, loader, atm, opts, epoch, advs, writer, loss_type_overwrite=None, privacy_eval=False):
    ...
    def get_private_label(input, target):
        private_labels = input[:, 3]
    ...
    for i, (input, target) in iterator:
        ...
        #input[:, 7] = 0
        input = ch.tensor(input, device='cuda:0', dtype=torch.float32)
        input.requires_grad_()
        target = ch.tensor(target, device='cuda:0', dtype=torch.long)
        private_labels = ch.tensor(private_labels, device='cuda:0', dtype=torch.long)
        # Compute Loss: eval
        if not is_train:
            # if adv_mi_type == 'lo':
