...
class WrappedEncoder(nn.Module):
    def __init__(self, args, dataset):
        super().__init__()
        self.enc = encoder.basic_encoder(num_classes=2)
    ...
    def _get_z_size(self, args):
        if (args.task == 'train-encoder') or (args.task == 'train-classifier') or (args.task == 'train-privacy')\
                or (args.task == 'train-all') or (args.task == 'train-privacy_classifier'):
            return 10
        else:
            return 10