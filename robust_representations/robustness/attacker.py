...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
                ...
                orig_input=None, use_best=True, return_image=True, est_grad=None, private_labels=None, args=None):
        """
        ...
        def calc_loss(inp, target, private_labels=None, args=None):
            '''
            ...
            if custom_loss:
                return custom_loss(self.model, inp, target, private_labels=private_labels, args=args)
            output = self.model(inp)

            return criterion(output, target), output

        # Main function for making adversarial examples
        def get_adv_examples(x, args=None):
            ...
            for _ in iterator:
                ...
                losses, out = calc_loss(step.to_image(x), target, private_labels=private_labels, args=args)
                ...
                with ch.no_grad():
                    argss = [losses, best_loss, x, best_x]
                    best_loss, best_x = replace_best(*argss) if use_best else (losses, x)
            ...
            losses, _ = calc_loss(step.to_image(x), target, private_labels=private_labels)
            argss = [losses, best_loss, x, best_x]
            best_loss, best_x = replace_best(*argss)
        ...
        if random_restarts:
            ...
            adv_ret = get_adv_examples(x, args=args)
...
class AttackerModel(ch.nn.Module):
    ...
    def forward(self, inp, target=None, make_adv=False, with_latent=False,
                ...
                get_feature_only=False, private_labels=None, privacy_eval=False, **attacker_kwargs):
        """
        ...
        if make_adv:
            ...
            self.eval()
            adv = self.attacker(inp, target, private_labels=private_labels, **attacker_kwargs)
            if prev_training:
                self.train()
        ...
        if with_image:
            normalized_inp = inp # todo typo normalize
    ...
    def forward_custom(self, input, target=None, make_adv=False,
        ...
        if make_adv:
            ...
            self.eval()
            # adv_target = target
            adv = self.attacker(input, target, private_labels=private_labels, **attacker_kwargs, should_normalize=True, args=args)
            if prev_training:
                self.train()
        ...
        if with_image:
            ...
            output = self.attacker.model.forward(input=input,
                                                 target=target,
                                                 loss_type=loss_type,
                                                 detach=detach,
